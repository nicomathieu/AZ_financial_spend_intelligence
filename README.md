...
| **LiteLLM proxy** | All LLM calls route through a self-hosted LiteLLM proxy (`LITELLM_HOST`). Invoice data never leaves the organisation's infrastructure — required for GDPR and data residency policy. Model and host are read from `.env` at call time; no credentials or URLs hardcoded in the repo. |

**LLM routing.** All LLM calls route through a self-hosted LiteLLM proxy (`LITELLM_HOST` + `LITELLM_MODEL` in `.env`). No credentials or model names are hardcoded in the repository — the server reads them lazily at call time so the codebase is environment-agnostic.
...
**Cosine similarity over 9 chunks.** NumPy dot product on L2-normalised vectors is O(9) — instantaneous. In production with a large policy corpus: hybrid BM25 + dense retrieval with Reciprocal Rank Fusion (RRF) over a managed search service (e.g. OpenSearch, Elasticsearch).
...
- **`APPROVAL_LEVEL_VIOLATION` is indicative only** — role mapping uses a mock `approver_roles.json`. Inferring roles from observed invoice behaviour would be circular. Production fix: live IAM/HR system lookup.
...
- **Embedder uses TF-IDF fallback** — `sentence-transformers` may be blocked by corporate SSL inspection proxies at startup. Production fix: a hosted embedding API for data residency compliance.
...
> Offline eval as a CI gate on every deployment, online eval as scheduled monitoring in production.
...
- SSO with role-based access (analyst / controller / auditor)
...
1. Implement real approval-level validation with live IAM/HR integration — replacing `approver_roles.json` with a live lookup
...
5. Production auth — SSO with auditor / controller / analyst roles
